...
void Dacler::set(Map& map, MainClass& object)
{
	if (seted == false)
	{
		float mouse_position_x = (float)Mouse::getPosition().x / object.m_window.getSize().x * object.m_camera.getSize().x;
		float mouse_position_y = (float)Mouse::getPosition().y / object.m_window.getSize().y * object.m_camera.getSize().y;
